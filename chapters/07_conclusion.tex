...
This is consistent with the \gls{edc} stack verifying a presentation once per exchange and then proceeding against an established context, and with BaSyx validating per message (\cref{sec:x1-overhead}). The per-hop traces collected during the campaign are the evidence that would confirm it directly.
...
absolute latencies are therefore lower than a distributed deployment would show. What carries beyond this setup is the ordering of the connectors and the shape of their curves, not the millisecond values.
